...
def subtract_blue_from_green():
    ...
    green_blue = green.astype(np.float32) - blue.astype(np.float32)

    # Ensure the values are within the valid range [0, 255]
    subtracted_image = np.clip(green_blue, 0, 255)

    # Convert the resulting image back to uint8
    subtracted_image = np.uint8(green_blue)
...
def k_channel_of_cmyk():
    ...
    rgb_image = img.astype(np.float32) / 255.0

    # Calculate CMY channels
    cmy_image = 1 - rgb_image

    # Extract CMY channels
...
def remove_noisy_objects(thresh_img, num):
    ...
    if num == 5:
        ...
        result_img_resized = result_img_pil.resize((300, 300))
        result_img_resized = ImageTk.PhotoImage(result_img_resized)

        ThresholdImageFrame5_image_label = Label(ThresholdImageFrame5, image=result_img_resized)
        ThresholdImageFrame5_image_label.image = result_img_resized
        ...
        ThresholdImageFrame6_image_label = Label(ThresholdImageFrame6, image=result_img_resized)
        ThresholdImageFrame6_image_label.image = result_img_resized
        ...
        ThresholdImageFrame7_image_label = Label(ThresholdImageFrame7, image=result_img_resized)
        ThresholdImageFrame7_image_label.image = result_img_resized
        ...
        ThresholdImageFrame8_image_label = Label(ThresholdImageFrame8, image=result_img_resized)
        ThresholdImageFrame8_image_label.image = result_img_resized
...
def identify_nymphs():
    ...
    num = count_objects(thresh_img_without_noise)
    label5_text.set(num)
...
label5_text.set("--")
Label5 = Label(infoFrame, textvariable=label5_text)

label6_text = StringVar()
label6_text.set("--")
Label6 = Label(infoFrame, textvariable=label6_text)

label7_text = StringVar()
label7_text.set("--")
Label7 = Label(infoFrame, textvariable=label7_text)

label8_text = StringVar()
label8_text.set("--")
Label8 = Label(infoFrame, textvariable=label8_text)
